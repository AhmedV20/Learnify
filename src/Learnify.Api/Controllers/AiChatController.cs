...
using Learnify.Api.Controllers;
...
namespace Learnify.Api.Controllers
{
    ...
    {
        /// <summary>
        /// The AI model name (optional). Defaults to "meta-llama/Llama-3.1-8B-Instruct" if not provided.
        /// </summary>
        [JsonPropertyName("model")]
        public string? Model { get; set; } // Optional - defaults to Llama 3.1 8B if not provided
    ...
    [ApiController]
    [ApiVersion("1.0")] // Marks this class as an API controller
    [Route("api/v{version:apiVersion}/[controller]")] // Defines the base route for this controller (e.g., /api/AiChat)
    public class AiChatController : ControllerBase
    {
        ...
        // Default AI model used when the frontend doesn't specify one
        private const string DefaultAiModel = "openai/gpt-oss-120b";
        ...
        public async Task<IActionResult> Post([FromBody] FrontendInferenceRequestDto requestDto)
        {
            ...
            // Only 'inputs' is required - model is optional with a default.
            if (requestDto == null || string.IsNullOrWhiteSpace(requestDto.Inputs))
            {
                return BadRequest("Invalid request body. 'inputs' is required and should be a non-empty string.");
            }

            // Use the provided model or fall back to the default
            var modelToUse = string.IsNullOrWhiteSpace(requestDto.Model) ? DefaultAiModel : requestDto.Model;

            // Create an HttpClient instance using the factory.
            var httpClient = _httpClientFactory.CreateClient();

            // Define the target URL for the Hugging Face Router's OpenAI-compatible endpoint.
            // This is the unified endpoint for all providers. The model is specified in the request body.
            var routerApiUrl = "https://router.huggingface.co/v1/chat/completions";
            ...
            var systemPrompt = "You are an AI assistant for the Learnify e-learning platform. Only answer questions about Learnify, its features, its courses, and its instructors. If a user asks about anything else, politely decline and redirect them to Learnify topics. ( if user ask else questions, answer him corect info )";
            var routerRequestBody = new RouterApiCompletionRequest
            {
                Model = modelToUse, // Use provided model or default to Llama 3.1 8B
